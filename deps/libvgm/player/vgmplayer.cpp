...
UINT8 VGMPlayer::GetSongDeviceInfo(std::vector<PLR_DEV_INFO>& devInfList) const
{
	...
	for (curDev = 0; curDev < _devCfgs.size(); curDev++)
	{
		...
		else
		{
			...
			// PARCHE LOCAL (AOLIB) -- ver
			// deps/patches/README.md "libvgm: NULL deref en
			// GetSongDeviceInfo() ante chip totalmente no registrado".
			// SndEmu_GetDevDecl() devuelve NULL si 'devType' no está
			// registrado en sndEmu_Devices[] (SNDDEV_<chip> nunca
			// definido, no solo EC_<chip>_* ausente) -- ocurre con
			// cualquier fichero VGM real que declare un chip que el
			// build todavía no active (medido con contenido real:
			// VGM de Sharp X68000 con YM2151+MSM6258 antes de que
			// MSM6258 se registrara). Original: 'devDecl->linkDevIDs(...)'
			// sin comprobar NULL primero.
			const DEVLINK_IDS* dlIds = (devDecl != NULL)
	...
	for (curDev = 0, diIdx = 0; curDev < _devCfgs.size(); curDev ++)
	{
		...
		else
		{
			...
			// PARCHE LOCAL (AOLIB) -- mismo hallazgo que
			// el bucle anterior, ver deps/patches/README.md. Aquí
			// 'devInf.devDecl' (no una variable local 'devDecl') es la que
			// puede ser NULL si SndEmu_GetDevDecl() no encuentra el tipo.
			const DEVLINK_IDS* dlIds = (devInf.devDecl != NULL)
				? devInf.devDecl->linkDevIDs(dCfg)
				: NULL;
			if (dlIds != NULL && dlIds->devCount > 0)
			{
				size_t curLDev;
