...
/* ─── PARCHE LOCAL ──────────────────────────────────────────────────
   ORIGINAL: rvb_downbuf/rvb_upbuf/rvb_dbpos/rvb_ubpos eran 'static' LOCALES de
   MixREVERBLeftRight(), es decir estado de proceso al que nadie podia
   llegar desde fuera. SPUinit() limpia s_chan, rvb, regArea y spuMem
   (spu.c:484-495), pero no podia limpiar esto: el historico del filtro de
   diezmado/interpolacion a 22 kHz del reverb sobrevivia intacto de una
   pista a la siguiente.

   Para la app de escritorio de aosdk es inofensivo (un proceso por pista);
   para un core de Libretro NO, porque un solo proceso reproduce un album
   entero.

   MEDIDO (F11, bisección por instantánea/restauración de los 113 estáticos
   de aosdk): estos son dos de los TRES simbolos sin los cuales el audio de
   PSF1 no es reproducible entre cargas -- los otros son root_cnts[].count
   y .target en psx_hw.c. Cargando tres veces el mismo "101 The Prelude" de
   FF7 en el mismo proceso, el audio divergia desde la muestra 8522
   (t=0,097 s) con >80 % de muestras distintas.

   Se suben a ambito de fichero para que RVBReset() pueda ponerlos a cero, y
   SPUinit() la llama. downcoeffs se queda dentro: es una tabla constante,
   no estado. */
static s32 rvb_downbuf[2][8];
static s32 rvb_upbuf[2][8];
static int rvb_dbpos=0, rvb_ubpos=0;

void RVBReset(void)
{
	memset(rvb_downbuf, 0, sizeof(rvb_downbuf));
	memset(rvb_upbuf,   0, sizeof(rvb_upbuf));
	rvb_dbpos = 0;
	rvb_ubpos = 0;
}
...
INLINE void MixREVERBLeftRight(s32 *oleft, s32 *oright, s32 inleft, s32 inright)
{
	...
	rvb_downbuf[0][rvb_dbpos]=inleft;
	rvb_downbuf[1][rvb_dbpos]=inright;
	rvb_dbpos=(rvb_dbpos+1)&7;

	if(rvb_dbpos&1) { // we work on every second left value: downsample to 22 khz
		if(spuCtrl&0x80) { // -> reverb on? oki
			...
			for(x=0; x<8; x++) {
				...
				INPUT_SAMPLE_L+=(rvb_downbuf[0][(rvb_dbpos+x)&7]*downcoeffs[x])>>8;
				INPUT_SAMPLE_R+=(rvb_downbuf[1][(rvb_dbpos+x)&7]*downcoeffs[x])>>8;
			}
			...
			INPUT_SAMPLE_R>>=(16-8);
			{
				...
				rvb_upbuf[0][rvb_ubpos]=rvb.iRVBLeft;
				rvb_upbuf[1][rvb_ubpos]=rvb.iRVBRight;
				rvb_ubpos=(rvb_ubpos+1)&7;
			} // Bracket hack(et).
		} else { // -> reverb off
			rvb.iRVBLeft=rvb.iRVBRight=0;
			return;
		...
		rvb_upbuf[0][rvb_ubpos]=0;
		rvb_upbuf[1][rvb_ubpos]=0;
		rvb_ubpos=(rvb_ubpos+1)&7;
	}
	{
		s32 retl=0,retr=0;
		for(x=0; x<8; x++) {
			retl+=(rvb_upbuf[0][(rvb_ubpos+x)&7]*downcoeffs[x])>>8;
			retr+=(rvb_upbuf[1][(rvb_ubpos+x)&7]*downcoeffs[x])>>8;
		}
