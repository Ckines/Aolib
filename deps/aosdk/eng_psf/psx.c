...
INLINE void mips_delayed_branch( UINT32 n_adr )
{
	...
	{
		/* EXPERIMENTO F5 (ver STATUS.md): que ocurre cuando la instruccion
		   del delay slot de un salto es, a su vez, un salto. En MIPS-I es
		   UNPREDICTABLE por especificacion, asi que la eleccion es una
		   decision de emulacion, no un hecho.
		   El original cae aqui incondicionalmente, con dos consecuencias:
		   (1) mips_commit_delayed_load() con delayr == REGPC escribe
		       mipscpu.r[32], UNO PASADO EL FINAL de r[32] -- pisa
		       cp0r[0]. Bug de memoria real, no detectable por ASan
		       (es intra-objeto).
		   (2) el segundo salto SOBREESCRIBE delayv: gana el ultimo.
		   ffxpatch.irx (Final Fantasy X) tiene exactamente ese patron en
		   su bucle de reintento (`b` con `jr $ra` en el delay slot), y con
		   "gana el ultimo" ese bucle no tiene salida. */
		if( mipscpu.delayr == REGPC )
		{
			/* gana el PRIMER salto: no se toca delayv ni se comete la
			   "carga retardada" (que aqui no es una carga, es el PC). */
			mipscpu.pc += 4;
			return;
		}
...
{
	/* ─── PARCHE LOCAL ──────────────────────────────────────────────
	   ORIGINAL: esta funcion era un no-op COMPLETO -- su cuerpo entero
	   estaba bajo #if 0 (el registro de estado para savestates de MAME,
	   que aosdk no usa). Y mips_reset(), la unica otra puerta de entrada,
	   NO limpia el contexto: hace un read-modify-write del CP0_SR
	   ANTERIOR y toca solo SR/RANDOM/PRID/pc/prevpc. Los 32 registros de
	   proposito general, HI/LO, el delay slot (delayr/delayv/op) y los
	   registros del GTE (cp2cr/cp2dr) sobrevivian intactos de una pista a
	   la siguiente.

	   Para la aplicacion de escritorio de aosdk eso da igual: un proceso
	   por pista. Para un core de Libretro NO -- un solo proceso reproduce
	   un album entero, y cada pista es un psf_start() nuevo sobre el mismo
	   'mipscpu'. psf_start() limpia psx_ram (eng_psf.c:184) pero nadie
	   limpiaba la CPU.

	   MEDIDO, no supuesto (F11): cargando tres veces el MISMO fichero
	   ("101 The Prelude" de FF7) en el mismo proceso, el audio divergia a
	   partir de la muestra 8522 (t=0,097 s) con el 80,55 % de las muestras
	   distintas y un delta maximo de 8122 (25 % del fondo de escala), y
	   cada carga sucesiva divergia de forma distinta. Descartados antes
	   por medicion: el precomputo de duraciones del .zip (un .minipsf
	   suelto divergia igual), psx_scratch sin limpiar (ponerlo a cero no
	   cambiaba nada) y la lectura de memoria del heap sin inicializar
	   (cuatro patrones de MALLOC_PERTURB_ daban el mismo MD5).

	   El reset se hace aqui, en mips_init(), y no en psf_start(), porque
	   'mipscpu' es static de este fichero (no hay forma de tocarlo desde
	   fuera) y porque mips_init() es la funcion que SIGNIFICA "inicializa
	   la CPU": la llaman tanto eng_psf.c:194 (PSF1) como eng_psf2.c:528 y
	   :613 (PSF2), asi que ambos motores heredan el arranque en frio.

	   Con el contexto a cero, mips_reset() deja CP0_SR = SR_BEV, que es
	   exactamente el estado de un R3000 recien reseteado -- antes ese
	   valor dependia de en que estado hubiera muerto la pista anterior. */
	memset( &mipscpu, 0, sizeof( mipscpu ) );
	mips_ICount = 0;
}
...
/* ─── Instrumentacion puntual (F5, ver STATUS.md) ────────────────────────
   Dos medidas que solo se pueden tomar dentro del despachador:
   (a) cuantas veces se entra en la funcion de "catch-up" del driver
       (DBG_PC_OUTER) y cuantas en el tick que esa funcion invoca en bucle
       (DBG_PC_TICK) -- el cociente ES el numero de ticks que el driver
       cree que debe recuperar, calculado a partir de nuestro sys_time.
   (b) la instruccion EXACTA que empuja $sp fuera de la RAM del IOP la
       primera vez, con su PC y su opcode -- sin esto solo se ve el
       resultado (un $sp imposible), nunca la causa.
   Ambas direcciones por defecto son las de ffxpatch.irx (IopSoundDriver
   v0214) cargado en 0x2d3b0; sobreescribibles por -D si se investiga
   otro contenido. Coste cero con DEBUG_SP_WATCH=0 (todo el bloque
   desaparece). */
#ifndef DEBUG_SP_WATCH
#define DEBUG_SP_WATCH (0)
#endif
#ifndef DBG_PC_OUTER
#define DBG_PC_OUTER (0x0003c538)
#endif
#ifndef DBG_PC_TICK
#define DBG_PC_TICK  (0x0002e080)
#endif

#if DEBUG_SP_WATCH
#include <stdio.h>
static UINT32 dbg_outer_hits = 0;
static UINT32 dbg_tick_hits = 0;
static UINT32 dbg_tick_at_last_outer = 0;
static UINT32 dbg_burst_max = 0;
static UINT32 dbg_burst_first = 0xffffffff;
static int    dbg_sp_reports = 0;
static int    dbg_sp_was_ok = 1;

/* El umbral correcto NO es el final de la RAM sino el FONDO de la pila
   inicial (initialSP = 0x801ffff0, fijado por eng_psf2.c): $sp por encima
   de ahi significa que ya se ha ejecutado un epilogo sin su prologo
   correspondiente -- un retorno de mas. Vigilar solo el limite de la RAM
   detecta el sintoma 16 bytes tarde, cuando la pila ya se ha desbordado. */
#ifndef DBG_SP_FLOOR
#define DBG_SP_FLOOR (0x001ffff0)
#endif
#define DBG_SP_OK(sp) ((((UINT32)(sp)) & 0x1fffff) <= DBG_SP_FLOOR)

void psx_dbg_sp_report(void);
void psx_dbg_sp_report(void)
{
	printf("[SPW] entradas en outer(%08x)=%u  tick(%08x)=%u\n",
		(unsigned)DBG_PC_OUTER, dbg_outer_hits,
		(unsigned)DBG_PC_TICK, dbg_tick_hits);
	printf("[SPW] ticks por llamada: primera rafaga=%u  maxima=%u  media=%.1f\n",
		(dbg_burst_first == 0xffffffff) ? 0 : dbg_burst_first,
		dbg_burst_max,
		dbg_outer_hits ? (double)dbg_tick_hits / (double)dbg_outer_hits : 0.0);
	fflush(stdout);
}
#endif
...
int mips_execute( int cycles )
{
	UINT32 n_res;
#if DEBUG_SP_WATCH
	UINT32 dbg_pc_before, dbg_op_before, dbg_sp_before;
#endif
...
#if DEBUG_SP_WATCH
		dbg_pc_before = mipscpu.pc;
		dbg_op_before = mipscpu.op;
		dbg_sp_before = mipscpu.r[ 29 ];
		if( mipscpu.pc == DBG_PC_OUTER )
		{
			UINT32 burst = dbg_tick_hits - dbg_tick_at_last_outer;
			if( dbg_outer_hits != 0 )
			{
				if( burst > dbg_burst_max ) dbg_burst_max = burst;
				if( dbg_burst_first == 0xffffffff ) dbg_burst_first = burst;
			}
			dbg_tick_at_last_outer = dbg_tick_hits;
			dbg_outer_hits++;
		}
		if( mipscpu.pc == DBG_PC_TICK ) dbg_tick_hits++;
#endif
...
skipinterp:
#if DEBUG_SP_WATCH
		if( DBG_SP_OK( dbg_sp_before ) && !DBG_SP_OK( mipscpu.r[ 29 ] ) )
		{
			if( dbg_sp_reports < 12 )
			{
				printf("[SPW] $sp POR ENCIMA DEL FONDO DE PILA: pc=%08x op=%08x  sp %08x -> %08x  (ra=%08x outer=%u tick=%u)\n",
					dbg_pc_before, dbg_op_before, dbg_sp_before,
					mipscpu.r[ 29 ], mipscpu.r[ 31 ],
					dbg_outer_hits, dbg_tick_hits);
				fflush(stdout);
			}
			dbg_sp_reports++;
			dbg_sp_was_ok = 0;
		}
		else if( !DBG_SP_OK( dbg_sp_before ) && DBG_SP_OK( mipscpu.r[ 29 ] ) )
		{
			dbg_sp_was_ok = 1;
		}
#endif
