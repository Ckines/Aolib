...
static void prepare_mixing(libvgmstream_priv_t* priv) {
    ...
    /* ORDER MATTERS: the resampler emits float, so it must be added before
     * the output sample format macro below -- otherwise that macro runs
     * first, the resampler is appended after it, and the output type ends
     * up float instead of the requested one. The buffer is then sized for
     * 2-byte samples while 4-byte ones are written into it.
     * It also has to precede mixing_enable, since mixing_setup ignores new
     * ops once the mixer is active. */
    if (cfg->output_sample_rate > 0) {
        mixing_set_resample(priv->vgmstream, cfg->output_sample_rate, cfg->resampler_type);
    }
...
static void update_format_info(libvgmstream_priv_t* priv) {
    ...
    fmt->native_sample_rate = v->sample_rate;
