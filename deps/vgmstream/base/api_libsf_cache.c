...
static int cache_read(void* user_data, uint8_t* dst, int64_t offset, int length) {
    ...
    if (pos >= priv->buf_offset && pos < priv->buf_offset + priv->valid_size) {
        size_t buf_limit;
        int buf_into = (int)(pos - priv->buf_offset);
    ...
    while (length > 0) {
        ...
        /* fill the buffer (pos now is beyond buf_offset) */
        priv->buf_offset = pos;
    ...
    priv->offset = pos; /* last read offset */
    return read_total;
}
