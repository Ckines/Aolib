...
VGMSTREAM* init_vgmstream_ubi_jade(STREAMFILE* sf) {
    ...
    if (!is_id32be(0x00,sf, "RIFF"))
        goto fail;
    /* the RIFF size must fit inside the file, but does not have to end
     * exactly at it: rips carry trailing padding. Beyond Good & Evil (PS2)
     * declares 3,191,910 + 8 in a 3,192,828 byte file, 910 bytes of tail,
     * and the equality test rejected the whole album. Short files are still
     * rejected, which is the case that actually means "not this format". */
