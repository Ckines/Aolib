...
VGMSTREAM* init_vgmstream_caf(STREAMFILE* sf) {
    ...
    while (offset < file_size) {
        ...
        /* a failed read returns -1, so 'offset += next_block' walks BACKWARDS
         * one byte per turn instead of advancing. Reachable whenever the
         * STREAMFILE reports the real file size while holding only a prefix,
         * which is what lazy .zip materialization does on purpose. */
        if (next_block <= 0 || offset + next_block <= offset)
            goto fail;
