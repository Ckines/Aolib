...
typedef struct {
    ...
    int sample_rate;                        // output sample rate (after resampling, if any)
    int native_sample_rate;                 // rate stored in the file, before any resampling
                                            // ** equals sample_rate when not resampling
...
typedef struct {
    ...
    int output_sample_rate;                 // resamples output to this rate, 0 = keep the stream's native rate
                                            // ** for players locked to a fixed rate; vgmstream's own resampler is used
    int resampler_type;                     // resampler quality when output_sample_rate is set, 0 = default
                                            // ** values are resampler_type_t (base/resampler.h)
